...
<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    @php
                        $homeRoute = route('vehicles.index');
                        if (auth()->check()) {
                            if (auth()->user()->isAdmin()) {
                                $homeRoute = route('admin.dashboard');
                            } elseif (auth()->user()->isManager()) {
                                $homeRoute = route('admin.vehicles');
                            }
                        }
                    @endphp
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if (!auth()->check() || (!auth()->user()->isAdmin() && !auth()->user()->isManager()))
                        <x-nav-link :href="route('vehicles.index')" :active="request()->routeIs('vehicles.index')" wire:navigate>
                            {{ __('Browse Vehicles') }}
                        </x-nav-link>
                    @endif
                    @auth
                        ...
                        <x-nav-link :href="route('admin.vehicles')" :active="request()->routeIs('admin.vehicles')" wire:navigate>
                        ...
                        @endif
                        <x-nav-link :href="route('notifications.index')" :active="request()->routeIs('notifications.index')" wire:navigate>
                            {{ __('Notifications') }}
                        </x-nav-link>
    ...
    <div :class="{ 'block': open, 'hidden': !open }" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('vehicles.index')" :active="request()->routeIs('vehicles.index')" wire:navigate>
                {{ __('Browse Vehicles') }}
            </x-responsive-nav-link>
            @auth
                ...
                @if (auth()->user()->isAdmin())
                    ...
                    <x-responsive-nav-link :href="route('admin.vehicles')" :active="request()->routeIs('admin.vehicles')" wire:navigate>
                        {{ __('Fleet Manager') }}
                    </x-responsive-nav-link>
